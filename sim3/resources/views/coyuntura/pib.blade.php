...
@section ('content') 
<div >
<DIV  ALIGN="center"><font size="4">Producto Interno Bruto<br>Nacional - Estado de Puebla</font><br>(Variación % real)</div>
<div ALIGN="center" id="top_x_div"></div>
</div>
<caption align='center'><button align='center'><a href='/impre/08/' target='_blank'><IMG SRC='/images/excel-xls-icon.png' WIDTH='50px' height='50px'></a></button><button align='center' onclick='mos()'><IMG SRC='/images/DEF.png' WIDTH='50px' height='50px'></button><button align='center' onclick='mostrardiv()'><IMG SRC='/images/meta.png' WIDTH='50px' height='50px'></button></caption>
</body>
</html>	
@stop
...
<td>Variable:</td>
<?php
		$vars=\sim\meta::where('id_variable', '=', '08')->take(1000)->get();
		?>
...
<?php
		$vars=\sim\def::where('id_variable', '=','08')->take(1000)->get();
		?>
			@foreach ($vars as $vari)
			<?php
